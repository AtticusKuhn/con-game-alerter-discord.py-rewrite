...
async def make_con_api_request(action, **kwargs):
    url_string = "&".join(
        list(map(lambda x: f'{x[0]}={x[1]}', kwargs.items())))
    hash_code = sha1(
        f'uberCon{action}{url_string}&authTstamp={authTstamp}&authUserID={auth_user_id}{auth_code}'
    ...
    data = {
        "data":
        base64.b64encode(
            f'{url_string}&authTstamp={authTstamp}&authUserID={auth_user_id}'.
            encode('ascii'))
    }
...
async def get_player_ranking(player_name):
    result = await make_con_api_request("searchUser", username=player_name)
    if len(result["result"]) == 0:
        return False
    player = result["result"][0]
    result2 = await make_con_api_request(
        "getUserDetailsFirefly",
...
async def get_global_games():
    ...
    # )
    hash_code = sha1(
        f'uberCongetInternationalGamesnumEntriesPerPage={numbers}&page=1&lang=en&isFilterSearch=false&openSlots=1&global=1&authTstamp={authTstamp}&authUserID=19999486{auth_code}'
        .encode('utf-8')).hexdigest()
    ...
    data = {
        "data":
        base64.b64encode(
            f'numEntriesPerPage={numbers}&page=1&lang=en&isFilterSearch=false&openSlots=1&global=1&authTstamp={authTstamp}&authUserID=19999486'
            .encode('ascii'))
    }
...
async def get_alliance(alliance_name):
    json_parsed_result = await make_con_api_request(
        "searchAlliance",
        name=alliance_name,
    )
...
async def get_players_in_game(game_id: int):
    return await make_con_api_request("getGame", gameID=game_id)


def get_session():
    print("get_session called")
    hash_code = sha1(f'authTstamp={authTstamp}&authUserID=19999486{auth_code}'.
                     encode('utf-8')).hexdigest()
    ...
    data = {
        "data":
        base64.b64encode(
            f'authTstamp={authTstamp}&authUserID=19999486'.encode('ascii'))
    }
    print("url", url)
    print("data", data)
